...
ggplot(df, mapping = aes(x = x, y = y, color = category)) +
  ...
  facet_wrap(~category) +
  labs(title = "Graphique avec plusieurs couches",
       x = "Variable X",
       y = "Variable Y") +
  ...
  ggplot(df, mapping = aes(x = x, y = y, color = category)) +
  geom_point(size = 1) + geom_smooth(method = "lm", linetype = "dashed",
...
#geom_polygon() : Ajoute des polygones (vous devez fournir des données supplémentaires pour définir les polygones)
